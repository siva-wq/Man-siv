...
const Subprofile = () => {
  ...
  const toggleLike = async (postId) => {
    ...
    try {
      ...
      const response = await axios.post("http://192.168.221.249/social_api/api/like.php", data, {
...
const total_likes=async(id)=>{
  ...
  useEffect(() => {
    const fetchProfileData = async () => {
      ...
      try {
        ...
        // Fetch follow info
        try {
          const followResponse = await axios.get(`http://192.168.221.249/social_api/api/getfollowercount.php?user_id=${userId}`);
        ...
        try {
          const followState = await axios.get(
            `http://192.168.221.249/social_api/api/isfollowing.php?user_id=${userId}&follower_id=${fid}`
          );
        ...
        const followerCount = await axios.get(
          `http://192.168.221.249/social_api/api/getfollowercount.php?user_id=${userId}`
        );
  ...
  const profilePicPath = user.profile_pic
    ? user.profile_pic.replace("C:/Users/durga/OneDrive/Desktop/Desktop/fed/React/myapp/public/", "")
    : "default.jpg";
        ...
        {posts.length > 0 ? (
          posts.map((post) => (
            <div key={post.id} className="post-card">
              ...
              {post.image && (
                <img
                  src={post.image.replace("C:/Users/durga/OneDrive/Desktop/Desktop/fed/React/myapp/public/", "")}
                  alt="Post"
