...
const PostFeed = () => {
  ...
  const islike = async (likedid) => {
    try {
      const response = await axios.get(
        `http://192.168.221.249/social_api/api/islike.php?user_id=${user}&post_id=${likedid}`
      );
  ...
  const updatecomment = async (id) => {
    try {
      ...
      const response = await axios.post(
        "http://192.168.221.249/social_api/api/comment.php",
        data1,
        {
  ...
  const getcomment = async (id) => {
    try {
      const response = await axios.get(
        `http://192.168.221.249/social_api/api/getcomment.php?post_id=${id}`
      );
  ...
  const fetchPosts = async () => {
    try {
      const response = await axios.get("http://192.168.221.249/social_api/api/post.php");
      ...
      const followerPromises = response.data.map(async (post) => {
        const followerResponse = await axios.get(
          `http://192.168.221.249/social_api/api/getfollowercount.php?user_id=${post.user_id}`
        );
        return {
